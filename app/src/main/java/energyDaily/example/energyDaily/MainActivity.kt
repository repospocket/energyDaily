...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        calendarView.setOnDayClickListener(object : OnDayClickListener {
            override fun onDayClick(eventDay: EventDay) {
                ...
                val time = clickedDayCalendar.get(Calendar.YEAR).toString() + " " + clickedDayCalendar.get( Calendar.MONTH ).toString() + " " + clickedDayCalendar.get(Calendar.DAY_OF_MONTH).toString()
                if (dateNotes[time].isNullOrEmpty()) {
                    ...
                    val toast =
                        Toast.makeText(applicationContext, dateNotes[time], Toast.LENGTH_LONG)
                    toast.show()
    ...
    fun tapcolor(view: View) {
        ...
        when (view.tag.toString()) {
            "green" -> list1.add(
                CalendarDay(calendar).apply {
                    backgroundDrawable =
                        ContextCompat.getDrawable(baseContext, android.R.color.holo_green_light)
                })
            "red" -> list1.add(
                CalendarDay(calendar).apply {
                    backgroundDrawable =
                        ContextCompat.getDrawable(baseContext, android.R.color.holo_red_light)
                })
            "blue" -> list1.add(
                CalendarDay(calendar).apply {
                    backgroundDrawable =
                        ContextCompat.getDrawable(baseContext, android.R.color.holo_blue_light)
                })
        ...
        saveData(datacolo, "task list")

    }

    private fun saveData(list: MutableMap<String, String>, sharedStr: String) {
        val sharedPreferences = getSharedPreferences("shared preferences", MODE_PRIVATE)
        val editor = sharedPreferences.edit()
        val json = Gson().toJson(list)
        editor.putString(sharedStr, json)
        editor.apply()
    }
